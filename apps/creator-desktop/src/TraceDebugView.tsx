...
export interface NarrativeReviewPanelProps {
  /** Narrative review from the trace, with 6 dimension scores. */
  review: NonNullable<PlayOnceReport["trace"]["narrative_review"]>;
}

// ---------------------------------------------------------------------------
// TraceDebugView (formerly RuntimeTracePanel)
//   UX simplification:  "Run Result Summary" default open,
//   "Technical Details" default closed, NarrativeReviewPanel extracted.
// ---------------------------------------------------------------------------
    ...
    <section aria-label="Proof And Trace Workspace" className="grid gap-5">
      <div className="grid gap-4 2xl:grid-cols-[minmax(0,1fr)_340px]">
        <div className="grid gap-4">
          ...
          {/* --------------------------------------------------------------- */}
          {/* Run Result Summary — default open                               */}
          {/* Contains action intent + world delta + errors                   */}
          {/* --------------------------------------------------------------- */}
          <Collapsible
            label="Run Result Summary"
            defaultOpen
          ...
          {/* CausalityGraph + TraceEvidence(4 sub-groups) + ToolMetadata +   */}
          {/* WorldDeltaEvidence + MediaReferenceList + NarrativeReviewPanel */}
          {/* + IssueList + DiagnosticList                                    */}
          {/* --------------------------------------------------------------- */}
          {trace ? (
            <CollapsibleSection
        ...
        {/* --------------------------------------------------------------- */}
        {/* Proof Evidence Panel — deduplicated                             */}
        {/* Removed duplicate trace.id, run_seed, prompt_version,            */}
        {/* provider_config_hash rows (already shown in badges / Run Result  */}
        {/* Summary). Kept package evidence + disclosure + static pkg.      */}
        {/* --------------------------------------------------------------- */}
        <aside
...
// ---------------------------------------------------------------------------
// NarrativeReviewPanel (extracted from ReviewScores, standalone export)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Internal sub-components (unchanged, moved from runtimeTraceView.tsx)
// ---------------------------------------------------------------------------
