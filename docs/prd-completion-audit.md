...
- Real text, image, TTS, and moderation provider calls are implemented behind explicit local configuration and redaction boundaries; hermetic provider doubles remain test-only. Vision remains deferred.
...
| T3.1 | AI output envelope, repair/validate, reproducibility metadata | `AgentOutputEnvelope`, `ReproducibilityMetadata`, `GenerationEvidence` in schema; `plotforge-agent` tests; CLI trace assertions | Covered for configured providers, offline pi-Agent scene planning, and hermetic test providers. |
...
| T3.3 | World Bible AI expansion and canon/forbidden facts workflow | `WorldGenerationRequest/Report`, provider-backed generation functions, storage/studio commands and tests | Requires an explicitly configured provider; fallback reports cannot modify project source. |
...
| T4.1 | Scene image persistence and reference reuse | `plotforge-agent` image provider pipeline, `plotforge-media` registry, scene background asset refs, export reachability tests | Covered with configured providers, cached references, explicit fallback metadata, and test-only provider doubles. |
