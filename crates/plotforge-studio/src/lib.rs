...
/// provider. The generated bytes are discarded; only a redaction-safe probe
/// result is returned to the caller.
pub fn test_image_provider(id: String) -> StudioCommandResult<ProviderTestResult> {
    let registry =
        plotforge_agent::load_provider_registry().map_err(|source| StudioCommandError {
            code: "test_image_provider_load".into(),
            message: source.to_string(),
        })?;
    let entry = registry
        .image_providers
    ...
    if !entry.enabled {
        return Ok(ProviderTestResult {
            ok: false,
            message: "image provider is disabled; enable it before testing".into(),
        });
    }
    let provider =
        plotforge_agent::build_image_provider(entry).map_err(|source| StudioCommandError {
            code: "test_image_provider_build".into(),
            message: redact_trace_text(&source.to_string()),
        })?;
    Ok(probe_image_provider(provider.as_ref()))
}

/// Adds or updates (by `id`) a TTS provider entry in the user-global
...
/// provider. Audio bytes are discarded and provider errors are redacted again
/// at the Studio boundary before reaching the UI.
pub fn test_tts_provider(id: String) -> StudioCommandResult<ProviderTestResult> {
    let registry =
        plotforge_agent::load_provider_registry().map_err(|source| StudioCommandError {
            code: "test_tts_provider_load".into(),
            message: source.to_string(),
        })?;
    let entry = registry
        .tts_providers
    ...
    if !entry.enabled {
        return Ok(ProviderTestResult {
            ok: false,
            message: "TTS provider is disabled; enable it before testing".into(),
        });
    }
    let provider =
        plotforge_agent::build_tts_provider(entry).map_err(|source| StudioCommandError {
            code: "test_tts_provider_build".into(),
            message: redact_trace_text(&source.to_string()),
        })?;
...
fn validate_media_provider_fields(
    ...
    enabled: bool,
) -> Result<(), plotforge_agent::TextProviderConfigError> {
    plotforge_agent::TextProviderConfig {
        enabled,
        provider: id.into(),
        model: model.into(),
...
fn upsert_image_provider_entry(registry: &mut ProviderRegistry, entry: ImageProviderEntry) {
        ...
        .find(|provider| provider.id == entry.id)
    {
        *existing = entry;
        ...
        registry.image_providers.push(entry);
    }
}

fn upsert_tts_provider_entry(registry: &mut ProviderRegistry, entry: TtsProviderEntry) {
    if let Some(existing) = registry
        .tts_providers
        .iter_mut()
        .find(|provider| provider.id == entry.id)
...
fn probe_image_provider(provider: &dyn plotforge_agent::ImageProvider) -> ProviderTestResult {
    let request = plotforge_agent::ImageGenerationRequest {
        scene_key: "connection-test".into(),
        prompt: "probe".into(),
        output_path: "connection-test.png".into(),
    };
    match provider.generate(&request) {
        Ok(_) => ProviderTestResult {
            ok: true,
            message: "minimal image generation completed".into(),
        },
...
fn probe_tts_provider(provider: &dyn plotforge_agent::TtsProvider) -> ProviderTestResult {
    let request = plotforge_agent::TtsRequest {
        target: plotforge_agent::TtsTarget::Scene {
            scene_key: "connection-test".into(),
        },
        text: "probe".into(),
        voice: String::new(),
        output_path: "connection-test.mp3".into(),
        asset_kind: plotforge_schema::AssetKind::Audio,
    };
...
mod tests {
  ...
  "entries": [
    ...
    fn sample_image_provider_entry(id: &str) -> ImageProviderEntry {
        ImageProviderEntry {
            id: id.into(),
            endpoint_url: "https://example.invalid/v1".into(),
            model: "gpt-image-test".into(),
            credential_env_var: "PLOTFORGE_IMAGE_TEST_KEY".into(),
            enabled: false,
    ...
    fn sample_tts_provider_entry(id: &str) -> TtsProviderEntry {
        TtsProviderEntry {
            id: id.into(),
            endpoint_url: "https://example.invalid/v1".into(),
            model: "gpt-tts-test".into(),
            credential_env_var: "PLOTFORGE_TTS_TEST_KEY".into(),
            enabled: false,
            voice: "coral".into(),
            format: "mp3".into(),
        }
    ...
    fn upsert_image_provider_succeeds() {
        ...
        .expect("valid image provider fields");
        let mut registry = ProviderRegistry::default();

        upsert_image_provider_entry(&mut registry, entry.clone());

        assert_eq!(registry.image_providers, vec![entry]);
    }

    #[test]
    fn upsert_image_provider_rejects_secret_endpoint_and_credential_value() {
        let mut endpoint_secret = sample_image_provider_entry("bad-image-endpoint");
        endpoint_secret.endpoint_url = "https://example.invalid/v1?key=sk-secret".into();
        let error = upsert_image_provider(endpoint_secret)
            .expect_err("endpoint credentials must be rejected before persistence");
        assert_eq!(error.code, "upsert_image_provider_invalid");

        let mut credential_value = sample_image_provider_entry("bad-image-credential");
        credential_value.credential_env_var = "sk-secret-value".into();
    ...
    #[test]
    fn delete_image_provider_not_found_is_explicit() {
        let error = delete_image_provider("pf-image-provider-absent-9f3c7a".into())
            .expect_err("missing image provider must error");
        assert_eq!(error.code, "image_provider_not_found");
    }

    struct SecretLeakingImageProvider;

    impl plotforge_agent::ImageProvider for SecretLeakingImageProvider {
        fn generate(
            &self,
            _request: &plotforge_agent::ImageGenerationRequest,
        ) -> Result<plotforge_agent::ImageGenerationResponse, plotforge_agent::ImageProviderError>
        {
            Err(plotforge_agent::ImageProviderError::provider(
                "probe_failed",
                "upstream echoed sk-image-secret",
            ))
        }
    }

    #[test]
    fn test_image_provider_redacts_error() {
        let result = probe_image_provider(&SecretLeakingImageProvider);

        assert!(!result.ok);
        assert!(!result.message.contains("sk-image-secret"));
        assert!(contains_secret_marker_text("sk-image-secret"));
    ...
    fn upsert_tts_provider_succeeds() {
        ...
        .expect("valid TTS provider fields");
        let mut registry = ProviderRegistry::default();

        upsert_tts_provider_entry(&mut registry, entry.clone());

        assert_eq!(registry.tts_providers, vec![entry]);
    }

    #[test]
    fn upsert_tts_provider_rejects_secret_endpoint_and_credential_value() {
        let mut endpoint_secret = sample_tts_provider_entry("bad-tts-endpoint");
        endpoint_secret.endpoint_url = "https://example.invalid/v1?token=sk-secret".into();
        let error = upsert_tts_provider(endpoint_secret)
            .expect_err("endpoint credentials must be rejected before persistence");
        assert_eq!(error.code, "upsert_tts_provider_invalid");

        let mut credential_value = sample_tts_provider_entry("bad-tts-credential");
        credential_value.credential_env_var = "sk-secret-value".into();
        let error = upsert_tts_provider(credential_value)
            .expect_err("credential values are not valid env-var names");
        assert_eq!(error.code, "upsert_tts_provider_invalid");
    }

    #[test]
    fn delete_tts_provider_not_found_is_explicit() {
        let error = delete_tts_provider("pf-tts-provider-absent-9f3c7a".into())
            .expect_err("missing TTS provider must error");
        assert_eq!(error.code, "tts_provider_not_found");
    }

    struct SecretLeakingTtsProvider;

    impl plotforge_agent::TtsProvider for SecretLeakingTtsProvider {
        fn synthesize(
            &self,
            _request: &plotforge_agent::TtsRequest,
        ) -> Result<plotforge_agent::TtsProviderOutput, plotforge_agent::TtsProviderError> {
            Err(plotforge_agent::TtsProviderError::provider(
                "probe_failed",
                "upstream echoed sk-tts-secret",
            ))
        }
    }

    #[test]
    fn test_tts_provider_redacts_error() {
        let result = probe_tts_provider(&SecretLeakingTtsProvider);
