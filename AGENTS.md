...
- The user-global Provider registry (`~/.plotforge/providers.json` or the platform config-dir equivalent) and the Skills library (`~/.plotforge/skills/`, `~/.plotforge/skill-index.json`) are user-global local-only configuration; they never enter project source, contracts, traces, or export packages. Credentials are referenced indirectly by environment-variable name (`credential_env_var`); the registry never stores credential values.
