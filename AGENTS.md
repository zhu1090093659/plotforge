...
- `docs/design/ui-mockups/agent-native-v1/` PNGs and README are historical development-time visual references only; they are not wired into the running Studio UI and must not be referenced from `apps/creator-desktop/src`. Use them only when revisiting the original design intent for the Agent Mesh, Command Center, Director Mode, proof, trace, and export package workflows.
